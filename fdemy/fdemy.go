...
func (u *Udemy) getLectureTitle(lectureID int) string {
...
func (u *Udemy) getDownloadURL(asset Asset) (string, error) {
	if asset.ID == 0 {
		return "", errors.New("invalid asset ID")
	}

	if asset.LectureID == 0 {
		return "", errors.New("invalid lecture ID ")
	}

	if asset.AssetType != "video" {
		return "", errors.New("asset is not a video")
	}
	url := fmt.Sprintf("https://www.udemy.com/api-2.0/users/me/subscribed-courses/%d/lectures/%d", u.CourseID.ID, asset.LectureID)
	...
	if !ok || len(sources) == 0 {
		return "", errors.New("no video sources found ")
	}

	source := sources[0].(map[string]interface{})
	downloadURL, ok := source["download_url"].(string)
	if !ok {
...
func (u *Udemy) GetDownloadLink(asset Asset) error {
	...
	lectureTitle := u.getLectureTitle(asset.LectureID)

	fileName := fmt.Sprintf("%s/%s - %s.mp4", u.CourseID.Title, lectureTitle, u.Resolution)
	...
	downloadURL, err := u.getDownloadURL(asset)
	if err != nil {
		return err
	}
	err = u.Download(downloadURL, asset)
	if err != nil {
		return err
	}
...
func (u *Udemy) startDownloading(courseAsset []Asset, courseTitle string) {
	...
	res := u.NewRequest("GET", GetCoursesURL)
	defer res.Body.Close()
	...
	for i := range response.Results {
		if strconv.Itoa(response.Results[i].ID) == u.SelectedCourseID {
			courseTitle = response.Results[i].Title
			break
		}
	}

	for l := range courseAsset {
		if courseAsset[l].ID != 0 {
			err := u.GetDownloadLink(courseAsset[l])
			if err == nil {
				err = os.Rename(strconv.Itoa(courseAsset[l].ObjectIndex)+"."+courseAsset[l].Title+".mp4", courseTitle+"/"+strconv.Itoa(courseAsset[l].ObjectIndex)+"."+courseAsset[l].Title+".mp4")
				if err != nil {
					fmt.Println("[x] Error renameing file : ", err)
				}
			}
		}
	}

	err := os.Mkdir(courseTitle, 0755)
	if err != nil {
		log.Fatal(err)
	}
}
